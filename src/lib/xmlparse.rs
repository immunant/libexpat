...
unsafe fn expat_free(mut parser: XML_Parser, mut ptr: *mut c_void, mut sourceLine: c_int) {
...
unsafe extern "C" fn expat_free_test_shim(parser: XML_Parser, ptr: *mut c_void, sourceLine: c_int) {
...
unsafe fn expat_realloc(
    ...
    if isIncrease {
        ...
        assert!((*rootParser).m_alloc_tracker.bytesAllocated >= absDiff as XmlBigCount);
        (*rootParser).m_alloc_tracker.bytesAllocated = (*rootParser)
            .m_alloc_tracker
            .bytesAllocated
...
unsafe extern "C" fn parserCreate(
    ...
    assert!((*rootParser_0).m_parentParser.is_null());
    assert!(
...
unsafe extern "C" fn externalEntityInitProcessor2(
    ...
    let mut tok: c_int =
        (*(*parser).m_encoding).scanners[1]((*parser).m_encoding, start, end, &raw mut next);
    match tok {
        XML_TOK_BOM => {
...
unsafe extern "C" fn externalEntityInitProcessor3(
    ...
    let mut next: *const c_char = start;
    (*parser).m_eventPtr = start;
    tok = (*(*parser).m_encoding).scanners[1]((*parser).m_encoding, start, end, &raw mut next);
...
unsafe extern "C" fn doContent(
    ...
    *eventPP = s;
    loop {
        ...
        let mut tok: c_int = (*enc).scanners[1](enc, s, end, &raw mut next);
...
unsafe extern "C" fn doCdataSection(
    ...
    *startPtr = null::<c_char>();
    loop {
        let mut next: *const c_char = s;
        let mut tok: c_int = (*enc).scanners[2](enc, s, end, &raw mut next);
...
unsafe extern "C" fn doIgnoreSection(
    ...
    *eventPP = s;
    *startPtr = null::<c_char>();
    tok = (*enc).scanners[3](enc, s, end, &raw mut next);
...
unsafe extern "C" fn entityValueInitProcessor(
    ...
    (*parser).m_eventPtr = start;
    loop {
        tok = (*(*parser).m_encoding).scanners[0]((*parser).m_encoding, start, end, &raw mut next);
        (*parser).m_eventEndPtr = next;
        if tok <= 0 {
...
unsafe extern "C" fn externalParEntProcessor(
    ...
    let mut next: *const c_char = s;
    let mut tok: c_int = 0;
    tok = (*(*parser).m_encoding).scanners[0]((*parser).m_encoding, s, end, &raw mut next);
    if tok <= 0 {
        if (*parser).m_parsingStatus.finalBuffer == 0 && tok != XML_TOK_INVALID {
            *nextPtr = s;
            return XML_ERROR_NONE;
        ...
        s = next;
        tok = (*(*parser).m_encoding).scanners[0usize]((*parser).m_encoding, s, end, &raw mut next);
    }
    (*parser).m_processor = Some(
        prologProcessor
            as unsafe extern "C" fn(
...
unsafe extern "C" fn doProlog(
    ...
    loop {
        ...
        *eventEndPP = next;
        ...
        match role {
            ...
            6 => {
                ...
                if (*parser).m_startDoctypeDeclHandler.is_some() {
                    ...
                    if (*enc).isPublicId(enc, s, next, eventPP) == 0 {
            ...
            9 => {
                if (*enc).predefinedEntityName(enc, s, next) != 0 {
                    (*parser).m_declEntity = null_mut::<ENTITY>();
        ...
        s = next;
        tok = (*enc).scanners[0](enc, s, end, &raw mut next);
    }
}
...
unsafe extern "C" fn appendAttributeValue(
    ...
    loop {
        let mut next: *const c_char = ptr;
        let mut tok: c_int = (*enc).literalScanners[0](enc, ptr, end, &raw mut next);
...
unsafe extern "C" fn storeEntityValue(
    ...
    's_35: loop {
        next = entityTextPtr;
        let mut tok: c_int =
            (*enc).literalScanners[1](enc, entityTextPtr, entityTextEnd, &raw mut next);
...
unsafe fn testingAccountingGetCountBytesDirect(mut parser: XML_Parser) -> c_ulonglong {
    if parser.is_null() {
        return 0u64;
    }
...
unsafe fn testingAccountingGetCountBytesIndirect(mut parser: XML_Parser) -> c_ulonglong {
    if parser.is_null() {
        return 0u64;
    }
...
unsafe fn unsignedCharToPrintable(mut c: c_uchar) -> *const c_char {
    match c as c_int {
...
unsafe extern "C" fn unsigned_char_to_printable_test_shim(c: c_uchar) -> *const c_char {
