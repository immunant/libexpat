...
fn expat_heap_increase_tolerable_impl(
    root_parser: &crate::src::xmlparse::XML_ParserStruct,
        ...
        < increase
    {
        ...
        if newTotal >= root_parser.m_alloc_tracker.activationThresholdBytes {
    ...
    let report_total = (!tolerable
        && root_parser.m_alloc_tracker.debugLevel >= 1 as ::core::ffi::c_ulong)
        .then_some(newTotal);
...
unsafe extern "C" fn processEntity(
    ...
    let mut freeEntityList: *mut *mut OPEN_INTERNAL_ENTITY =
        ::core::ptr::null_mut::<*mut OPEN_INTERNAL_ENTITY>();
    let mut uses_attribute_free_list = false;
    ...
    if uses_attribute_free_list && !openEntity.is_null() {
        // The nullable attribute free-list head was taken above.  Its node is
        // reused directly, preserving the original LIFO free-list behavior.
    } else if !uses_attribute_free_list && !(*freeEntityList).is_null() {
...
fn pool_append_char(pool: &mut STRING_POOL, value: crate::expat_external_h::XML_Char) -> bool {
    if pool.is_full() && unsafe { poolGrow(pool) } == 0 {
        return false;
    }
