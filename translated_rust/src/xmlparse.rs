...
impl BindingStorage {
    unsafe fn new(parser: crate::expat_h::XML_Parser, uri_capacity: usize) -> Option<Self> {
...
/// cursors for one token, not arbitrary foreign pointers.  A processing
/// instruction may additionally request the leading whitespace length before
...
unsafe fn event_name_length(
    ...
    let whitespace_after_name = trailing_delimiter_width.map(|trailing_width| {
        ...
        let data_end = source.len().checked_sub(trailing_width)?;
        let data = source.chars().get(name_end..data_end)?;
...
pub unsafe extern "C" fn XML_ParserFree(mut parser: crate::expat_h::XML_Parser) {
    ...
    let active_attribute_entities = std::mem::take(&mut parser.m_openAttributeEntities);
    for storage in active_attribute_entities.into_iter().rev() {
        storage.release(1972 as ::core::ffi::c_int);
    }
    let free_attribute_entities = std::mem::take(&mut parser.m_freeAttributeEntities);
            ...
            match tok {
                ...
                | crate::src::xmltok::XML_TOK_START_TAG_WITH_ATTS => {
                    ...
                    let Some(raw_name_len) =
                        event_name_length(parser, dtd, parser_events, enc, raw_name, next, None)
                    else {
                        return crate::expat_h::XML_ERROR_UNEXPECTED_STATE;
                    };
                ...
                | crate::src::xmltok::XML_TOK_EMPTY_ELEMENT_WITH_ATTS => {
                    ...
                    let Some(raw_name_len) =
                        event_name_length(parser, dtd, parser_events, enc, rawName, next, None)
                    else {
                        return crate::expat_h::XML_ERROR_UNEXPECTED_STATE;
                    };
                ...
                crate::src::xmltok::XML_TOK_END_TAG => {
                    {
                        ...
                        let Some(raw_name_len) =
                            event_name_length(parser, dtd, parser_events, enc, rawName_0, next, None)
                        else {
                            return crate::expat_h::XML_ERROR_UNEXPECTED_STATE;
                        };
...
unsafe extern "C" fn storeAtts(
    ...
    while i < n {
        ...
        let Some(name_len) = event_name_length(parser, dtd, parser_events, enc, name, attEnd, None)
        else {
            return crate::expat_h::XML_ERROR_UNEXPECTED_STATE;
        };
...
unsafe extern "C" fn callStoreEntityValue(
    ...
    loop {
        if parser.m_openValueEntities.is_none() {
            ...
            let entity: *mut ENTITY = match entity_ref.table {
                OpenEntityTable::General => {
                    lookup(parser_handle, &raw mut (*dtd).generalEntities, name.as_ptr(), 0)
                        .cast::<ENTITY>()
                }
                OpenEntityTable::Parameter => {
                    lookup(parser_handle, &raw mut (*dtd).paramEntities, name.as_ptr(), 0)
                        .cast::<ENTITY>()
                }
            };
            if entity.is_null() {
            ...
            if has_more {
                ...
                entityTrackingOnClose(parser_handle, entity, 6998 as ::core::ffi::c_int);
...
unsafe extern "C" fn reportComment(
    ...
    let callback = COMMENT_HANDLERS
        ...
        .cloned();
    if let Some(callback) = callback {
        callback.invoke(handler_arg!(parser), data);
    }
    poolClear(temp_pool);
    return 1 as ::core::ffi::c_int;
}
...
unsafe extern "C" fn setElementTypePrefix(
    ...
    let Some(prefix_len) = name_chars.iter().position(|&character| {
