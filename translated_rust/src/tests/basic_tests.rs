...
impl Iterator for AttrPairs {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        let name = read_attr_pair_entry(self.atts, self.index);
        if name.is_null() {
            return None;
        }

        let value = read_attr_pair_entry(self.atts, self.index + 1);
        self.index += 2;
        Some((name, value))
    }
}
...
impl Iterator for FeatureList {
    type Item = XML_Feature;

    fn next(&mut self) -> Option<Self::Item> {
        let feature = feature_entry(self.current)?;
...
fn format_attr_normalization_failure(
    ...
    rendered.extend_from_slice(b"attribute value not normalized: ");
    rendered.extend_from_slice(&copy_c_string_bytes(attrname.cast()));
    rendered.push(b'=');
    rendered.push(b'\'');
    rendered.extend_from_slice(&copy_c_string_bytes(value.cast()));
    rendered.push(b'\'');

    let copy_len = rendered.len().min(buffer.len().saturating_sub(1));
    for (slot, byte) in buffer.iter_mut().zip(rendered.into_iter()).take(copy_len) {
        *slot = byte as ::core::ffi::c_char;
    }
    if let Some(slot) = buffer.get_mut(copy_len) {
        *slot = 0;
    }
}
...
fn set_subtest_message(message: &str) {
    ...
    ffi_call!(set_subtest, bytes_as_c_char_ptr(b"%s\0"), message.as_ptr());
}

fn set_subtest_c_string(message: *const ::core::ffi::c_char) {
    set_subtest_message(&c_string_to_string_lossy(message));
}

fn set_split_subtest_message(split: ::core::ffi::c_int) {
    set_subtest_message(&format!("split at byte {split}"));
}
...
fn parse_single_bytes_c_string_expect_success(
    text: *const ::core::ffi::c_char,
    line: ::core::ffi::c_int,
) {
    ensure_parser_success(parse_single_bytes_c_string(text), line);
}
...
fn parser_set_end_doctype_decl_handler(handler: XML_EndDoctypeDeclHandler) {
    ffi_call2(XML_SetEndDoctypeDeclHandler, current_parser(), handler);
}

fn parser_set_entity_decl_handler(handler: XML_EntityDeclHandler) {
    ffi_call2(XML_SetEntityDeclHandler, current_parser(), handler);
}

fn parser_set_notation_decl_handler(handler: XML_NotationDeclHandler) {
    ffi_call2(XML_SetNotationDeclHandler, current_parser(), handler);
}

fn parser_set_element_decl_handler(handler: XML_ElementDeclHandler) {
    ffi_call2(XML_SetElementDeclHandler, current_parser(), handler);
}

fn parser_set_attlist_decl_handler(handler: XML_AttlistDeclHandler) {
    ffi_call2(XML_SetAttlistDeclHandler, current_parser(), handler);
}

fn parser_set_skipped_entity_handler(handler: XML_SkippedEntityHandler) {
    ffi_call2(XML_SetSkippedEntityHandler, current_parser(), handler);
}

fn parser_set_processing_instruction_handler(handler: XML_ProcessingInstructionHandler) {
    ffi_call2(
...
fn parser_set_comment_handler(handler: XML_CommentHandler) {
    ffi_call2(XML_SetCommentHandler, current_parser(), handler);
}

fn parser_set_comment_handler_for(parser: XML_Parser, handler: XML_CommentHandler) {
    ffi_call2(XML_SetCommentHandler, parser, handler);
}
...
fn parser_set_external_entity_ref_handler_arg(arg: *mut ::core::ffi::c_void) {
    ffi_call2(XML_SetExternalEntityRefHandlerArg, current_parser(), arg);
}
...
fn parser_use_parser_as_handler_arg() {
    ffi_call1(XML_UseParserAsHandlerArg, current_parser());
}
...
fn parse_single_bytes_array<const N: usize>(text: &[u8; N]) -> XML_Status {
    let len = ::core::ffi::c_int::try_from(N.saturating_sub(1))
        .expect("buffer length should fit into c_int");
    parse_single_bytes_with_final(text.as_ptr().cast(), len, XML_TRUE as ::core::ffi::c_int)
}

fn parse_single_bytes_array_expect_success<const N: usize>(
    text: &[u8; N],
    line: ::core::ffi::c_int,
) {
    ensure_parser_success(parse_single_bytes_array(text), line);
}
...
fn run_public_utf16_entity_test<const TEXT_LEN: usize, const EXTERNAL_LEN: usize>(
    test_name: &[u8],
    info_line: ::core::ffi::c_int,
    parse_line: ::core::ffi::c_int,
    text: [u8; TEXT_LEN],
    external_text: [u8; EXTERNAL_LEN],
    parse_len: ::core::ffi::c_int,
) {
    set_test_info(test_name, info_line);

    let text = c_char_array(text);
    let mut test_data = ExtTest2 {
        parse_text: external_text.as_ptr().cast(),
        parse_len,
        encoding: ::core::ptr::null::<XML_Char>(),
        storage: ::core::ptr::null_mut::<CharData>(),
    };
    let expected = bytes_as_xml_char_ptr(b"baz\0");
    ...
    parser_set_param_entity_parsing(XML_PARAM_ENTITY_PARSING_ALWAYS);
    parser_set_external_entity_ref_handler(external_entity_loader2_handler_for_tests());
    parser_set_user_data((&mut test_data as *mut ExtTest2).cast());
    parser_set_character_data_handler(ext2_accumulating_character_handler_for_tests());

    let text_len = ::core::ffi::c_int::try_from(TEXT_LEN - 1)
        .expect("UTF-16 test document length should fit into c_int");
    ensure_parser_success(
        parse_single_bytes_with_final(text.as_ptr(), text_len, XML_TRUE as ::core::ffi::c_int),
        parse_line,
    );
...
fn aborting_default_handler_for_tests() -> XML_DefaultHandler {
    Some(
...
fn suspending_end_handler_for_tests() -> XML_EndElementHandler {
    Some(
...
fn suspending_start_element_handler_for_tests() -> XML_StartElementHandler {
    Some(
...
fn suspending_comment_handler_for_tests() -> XML_CommentHandler {
    Some(
...
fn suspending_element_decl_handler_for_tests() -> XML_ElementDeclHandler {
    Some(
...
fn suspending_xml_decl_handler_for_tests() -> XML_XmlDeclHandler {
    Some(
...
fn external_entity_param_handler_for_tests() -> XML_ExternalEntityRefHandler {
    Some(external_entity_param)
}

fn external_entity_not_standalone_handler_for_tests() -> XML_ExternalEntityRefHandler {
    Some(external_entity_not_standalone)
}

fn external_entity_value_aborter_handler_for_tests() -> XML_ExternalEntityRefHandler {
    Some(external_entity_value_aborter)
}
...
fn dummy_notation_decl_handler_for_tests() -> XML_NotationDeclHandler {
    Some(dummy_notation_decl_handler)
}

fn dummy_element_decl_handler_for_tests() -> XML_ElementDeclHandler {
    Some(dummy_element_decl_handler)
}

fn dummy_skip_handler_for_tests() -> XML_SkippedEntityHandler {
    Some(dummy_skip_handler)
}
...
fn start_element_suspender_for_tests() -> XML_StartElementHandler {
    Some(
        start_element_suspender
            as unsafe extern "C" fn(
                *mut ::core::ffi::c_void,
...
fn record_element_end_handler_for_tests() -> XML_EndElementHandler {
    Some(
...
fn read_attr_pair_entry(atts: *mut *const XML_Char, index: usize) -> *const XML_Char {
    let mut entry = ::core::ptr::null::<XML_Char>();
    ffi_call3(
        memcpy,
        (&mut entry as *mut *const XML_Char).cast::<::core::ffi::c_void>(),
        atts.wrapping_add(index).cast::<::core::ffi::c_void>(),
        ::core::mem::size_of::<*const XML_Char>() as size_t,
    );
    entry
}

fn feature_entry(current: *const XML_Feature) -> Option<XML_Feature> {
    if current.is_null() {
        return None;
    }

    let mut feature = XML_Feature {
    ...
    ffi_call3(
        memcpy,
        (&mut feature as *mut XML_Feature).cast::<::core::ffi::c_void>(),
        current.cast::<::core::ffi::c_void>(),
        ::core::mem::size_of::<XML_Feature>() as size_t,
    );
    Some(feature)
}
...
fn ext2_accumulating_character_handler_for_tests() -> XML_CharacterDataHandler {
    Some(ext2_accumulate_characters)
}

fn external_entity_faulter2_handler_for_tests() -> XML_ExternalEntityRefHandler {
    Some(external_entity_faulter2)
}

fn configure_ext2_character_data_test(test_data: &mut ExtTest2, storage: &mut CharData) {
    char_data_init(storage);
    test_data.storage = storage;
    parser_set_external_entity_ref_handler(external_entity_loader2_handler_for_tests());
    parser_set_user_data((test_data as *mut ExtTest2).cast());
    parser_set_character_data_handler(ext2_accumulating_character_handler_for_tests());
}

fn configure_ext2_fault_test(test_data: &mut ExtFaults2) {
    parser_set_external_entity_ref_handler(external_entity_faulter2_handler_for_tests());
    parser_set_user_data((test_data as *mut ExtFaults2).cast());
}
...
extern "C" fn test_dtd_default_handling() {
    set_test_info(b"test_dtd_default_handling\0", 1381 as ::core::ffi::c_int);
    let text = bytes_as_c_char_ptr(
    ...
    parser_set_start_doctype_decl_handler(Some(
    ...
    parser_set_end_doctype_decl_handler(Some(
        dummy_end_doctype_handler as unsafe extern "C" fn(*mut ::core::ffi::c_void) -> (),
    ));
    ...
    parser_set_notation_decl_handler(Some(
    ...
    parser_set_element_decl_handler(Some(
    ...
    parser_set_processing_instruction_handler(Some(
        dummy_pi_handler
            as unsafe extern "C" fn(
                *mut ::core::ffi::c_void,
                *const XML_Char,
    ...
    parser_set_comment_handler(Some(
        dummy_comment_handler
            as unsafe extern "C" fn(*mut ::core::ffi::c_void, *const XML_Char) -> (),
    ));
    parser_set_start_cdata_section_handler(Some(
        dummy_start_cdata_handler as unsafe extern "C" fn(*mut ::core::ffi::c_void) -> (),
    ));
    parser_set_end_cdata_section_handler(Some(
        dummy_end_cdata_handler as unsafe extern "C" fn(*mut ::core::ffi::c_void) -> (),
    ));
    run_character_check(
        text,
        bytes_as_xml_char_ptr(b"\n\n\n\n\n\n\n<doc/>\0"),
        1402 as ::core::ffi::c_int,
    );
}
extern "C" fn test_dtd_attr_handling() {
    set_test_info(b"test_dtd_attr_handling\0", 1407 as ::core::ffi::c_int);
    let prolog = bytes_as_c_char_ptr(b"<!DOCTYPE doc [\n<!ELEMENT doc EMPTY>\n\0");
    let attr_data: [AttTest; 5] = [
            AttTest {
                definition: bytes_as_c_char_ptr(
                    b"<!ATTLIST doc a ( one | two | three ) #REQUIRED>\n]><doc a='two'/>\0",
                ),
                element_name: bytes_as_xml_char_ptr(b"doc\0"),
                attr_name: bytes_as_xml_char_ptr(b"a\0"),
                attr_type: bytes_as_xml_char_ptr(b"(one|two|three)\0"),
            ...
            AttTest {
                ...
                element_name: bytes_as_xml_char_ptr(b"doc\0"),
                attr_name: bytes_as_xml_char_ptr(b"a\0"),
                attr_type: bytes_as_xml_char_ptr(b"NOTATION(foo)\0"),
                default_value: ::core::ptr::null::<XML_Char>(),
                is_required: XML_FALSE as ::core::ffi::c_int,
            },
            AttTest {
                definition: bytes_as_c_char_ptr(b"<!ATTLIST doc a NOTATION (foo) 'bar'>\n]><doc/>\0"),
                element_name: bytes_as_xml_char_ptr(b"doc\0"),
                attr_name: bytes_as_xml_char_ptr(b"a\0"),
                attr_type: bytes_as_xml_char_ptr(b"NOTATION(foo)\0"),
                default_value: bytes_as_xml_char_ptr(b"bar\0"),
                is_required: XML_FALSE as ::core::ffi::c_int,
            },
            AttTest {
                definition: bytes_as_c_char_ptr(b"<!ATTLIST doc a CDATA '\xDB\xB2'>\n]><doc/>\0"),
                element_name: bytes_as_xml_char_ptr(b"doc\0"),
                attr_name: bytes_as_xml_char_ptr(b"a\0"),
                attr_type: bytes_as_xml_char_ptr(b"CDATA\0"),
                default_value: bytes_as_xml_char_ptr(b"\xDB\xB2\0"),
                is_required: XML_FALSE as ::core::ffi::c_int,
            },
            AttTest {
        ...
        .take_while(|test| !test.definition.is_null())
    {
        ...
        parser_set_user_data(test as *const AttTest as *mut ::core::ffi::c_void);
        ensure_parser_success(
        ...
        ensure_parser_success(
            parse_single_bytes_c_string(test.definition),
...
extern "C" fn test_empty_ns_without_namespaces() {
    set_test_info(
        b"test_empty_ns_without_namespaces\0",
...
extern "C" fn test_ns_in_attribute_default_without_namespaces() {
    set_test_info(
        b"test_ns_in_attribute_default_without_namespaces\0",
...
extern "C" fn test_long_cdata_utf16() {
    set_test_info(b"test_long_cdata_utf16\0", 1661 as ::core::ffi::c_int);
    let prefix = b"\0<\0?\0x\0m\0l\0 \0v\0e\0r\0s\0i\0o\0n\0=\0'\x001\0.\x000\0'\0 \0e\0n\0c\0o\0d\0i\0n\0g\0=\0'\0u\0t\0f\0-\x001\x006\0'\0?\0>\0<\0a\0>\0<\0!\0[\0C\0D\0A\0T\0A\0[";
    let repeated = b"\0A\0B\0C\0D\0E\0F\0G\0H\0I\0J\0K\0L\0M\0N\0O\0P";
    let suffix = b"\0]\0]\0>\0<\0/\0a\0>";
    let mut text: Vec<::core::ffi::c_char> = prefix
        .iter()
        .copied()
        .map(|byte| byte as ::core::ffi::c_char)
        .collect();
    for _ in 0..65 {
        text.extend(
            repeated
    ...
    let expected = std::ffi::CString::new("ABCDEFGHIJKLMNOP".repeat(65))
        .expect("expected UTF-16 CDATA text must not contain NUL");
    let mut storage = CharData {
        count: 0,
        data: [0; 2048],
    };
    ...
    let text_len = ::core::ffi::c_int::try_from(text.len())
        .expect("UTF-16 CDATA test length should fit into c_int");
    let buffer = current_parser_buffer(text_len);
    if buffer.is_null() {
        fail_test(
    ...
    ffi_call3(
        memcpy,
        buffer,
        text.as_ptr().cast::<::core::ffi::c_void>(),
        usize::try_from(text_len).expect("buffer length should fit into usize") as size_t,
    );
    ensure_parser_success(
        current_parser_parse_buffer(text_len, XML_TRUE as ::core::ffi::c_int),
        1726 as ::core::ffi::c_int,
    );
    char_data_check_xml_chars(&mut storage, expected.as_ptr().cast());
}
...
extern "C" fn test_user_parameters() {
    set_test_info(b"test_user_parameters\0", 2833 as ::core::ffi::c_int);
    let text = bytes_as_c_char_ptr(
        b"<?xml version='1.0' encoding='us-ascii'?>\n<!-- Primary parse -->\n<!DOCTYPE doc SYSTEM 'foo'>\n<doc>&entity;\0",
    );
    let epilog = bytes_as_c_char_ptr(b"<!-- Back to primary parser -->\n</doc>\0");

    unsafe_global_set!(g_comment_count, 0 as ::core::ffi::c_int);
    unsafe_global_set!(g_skip_count, 0 as ::core::ffi::c_int);
    unsafe_global_set!(g_xdecl_count, 0 as ::core::ffi::c_int);
    parser_set_param_entity_parsing(XML_PARAM_ENTITY_PARSING_ALWAYS);
    parser_set_xml_decl_handler(Some(xml_decl_handler));
    parser_set_external_entity_ref_handler(Some(external_entity_param_checker));
    parser_set_comment_handler(Some(data_check_comment_handler));
    parser_set_skipped_entity_handler(Some(param_check_skip_handler));
    parser_use_parser_as_handler_arg();
    parser_set_user_data(1 as ::core::ffi::c_int as *mut ::core::ffi::c_void);
    unsafe_global_set!(
        g_handler_data,
        current_parser() as *const ::core::ffi::c_void
    );

    ensure_parser_success(
        parser_parse(text, c_string_len(text), XML_FALSE as ::core::ffi::c_int),
        2854 as ::core::ffi::c_int,
    );
    if parser_set_param_entity_parsing(XML_PARAM_ENTITY_PARSING_NEVER) != 0 {
        fail_test(
            2857 as ::core::ffi::c_int,
            b"Changed param entity parsing policy while parsing\0",
        );
    }
    ensure_parser_success(
        parser_parse(epilog, c_string_len(epilog), XML_TRUE as ::core::ffi::c_int),
        2860 as ::core::ffi::c_int,
    );

    if unsafe_global_get!(g_comment_count) != 3 as ::core::ffi::c_int {
        fail_test(
            2862 as ::core::ffi::c_int,
            b"Comment handler not invoked enough times\0",
        );
    }
    if unsafe_global_get!(g_skip_count) != 1 as ::core::ffi::c_int {
        fail_test(
            2864 as ::core::ffi::c_int,
            b"Skip handler not invoked enough times\0",
        );
    }
    if unsafe_global_get!(g_xdecl_count) != 1 as ::core::ffi::c_int {
        fail_test(
            2866 as ::core::ffi::c_int,
            b"XML declaration handler not invoked\0",
        );
    }
}
extern "C" fn test_ext_entity_ref_parameter() {
    set_test_info(
        b"test_ext_entity_ref_parameter\0",
    ...
    parser_set_param_entity_parsing(XML_PARAM_ENTITY_PARSING_ALWAYS);
    parser_set_external_entity_ref_handler(Some(external_entity_ref_param_checker));
    parser_set_external_entity_ref_handler_arg(text.cast_mut().cast());
    unsafe_global_set!(g_handler_data, text as *const ::core::ffi::c_void);
    ensure_parser_success(
        parse_single_bytes_c_string(text),
    ...
    parser_set_param_entity_parsing(XML_PARAM_ENTITY_PARSING_ALWAYS);
    parser_set_external_entity_ref_handler(Some(external_entity_ref_param_checker));
    parser_set_external_entity_ref_handler_arg(::core::ptr::null_mut::<::core::ffi::c_void>());
    unsafe_global_set!(
        g_handler_data,
        current_parser() as *const ::core::ffi::c_void
    );
...
extern "C" fn test_predefined_entities() {
    set_test_info(b"test_predefined_entities\0", 3224 as ::core::ffi::c_int);
    let text = bytes_as_c_char_ptr(b"<doc>&lt;&gt;&amp;&quot;&apos;</doc>\0");
    let expected = bytes_as_xml_char_ptr(b"<doc>&lt;&gt;&amp;&quot;&apos;</doc>\0");
    let result = bytes_as_xml_char_ptr(b"<>&\"'\0");
    let mut storage: CharData = CharData {
        count: 0,
        data: [0; 2048],
    };
    ...
    char_data_init(&mut storage);
    parser_set_user_data((&raw mut storage).cast());
    parse_single_bytes_c_string_expect_success(text, 3238 as ::core::ffi::c_int);
    char_data_check_xml_chars(&mut storage, expected);
    parser_reset();
    run_character_check(text, result, 3244 as ::core::ffi::c_int);
}
extern "C" fn test_invalid_tag_in_dtd() {
    set_test_info(b"test_invalid_tag_in_dtd\0", 3256 as ::core::ffi::c_int);
    let text = bytes_as_c_char_ptr(b"<!DOCTYPE doc SYSTEM '004-1.ent'>\n<doc></doc>\n\0");

    parser_set_param_entity_parsing(XML_PARAM_ENTITY_PARSING_ALWAYS);
    parser_set_external_entity_ref_handler(external_entity_param_handler_for_tests());
    expect_failure(
        text,
        XML_ERROR_EXTERNAL_ENTITY_HANDLING,
        b"Invalid tag IN DTD external param not rejected\0",
        3263 as ::core::ffi::c_int,
    );
}
extern "C" fn test_not_predefined_entities() {
    set_test_info(
        b"test_not_predefined_entities\0",
        3268 as ::core::ffi::c_int,
    );
    let texts = [
        bytes_as_c_char_ptr(b"<doc>&pt;</doc>\0"),
        bytes_as_c_char_ptr(b"<doc>&amo;</doc>\0"),
        bytes_as_c_char_ptr(b"<doc>&quid;</doc>\0"),
        bytes_as_c_char_ptr(b"<doc>&apod;</doc>\0"),
        ::core::ptr::null::<::core::ffi::c_char>(),
    ];

    for &text in texts.iter().take_while(|text| !text.is_null()) {
        expect_failure(
            text,
            XML_ERROR_UNDEFINED_ENTITY,
            b"Undefined entity not rejected\0",
...
extern "C" fn test_ignore_section() {
    set_test_info(b"test_ignore_section\0", 3283 as ::core::ffi::c_int);
    let text = bytes_as_c_char_ptr(b"<!DOCTYPE doc SYSTEM 'foo'>\n<doc><e>&entity;</e></doc>\0");
    let expected = bytes_as_xml_char_ptr(b"<![IGNORE[<!ELEMENT e (#PCDATA)*>]]>\n&entity;\0");
    let mut storage = CharData {
        count: 0,
        data: [0; 2048],
    };
    ...
    parser_set_element_decl_handler(Some(
        dummy_element_decl_handler
            as unsafe extern "C" fn(*mut ::core::ffi::c_void, *const XML_Char, *mut XML_Content),
    ));
    parser_set_start_element_handler(Some(
        dummy_start_element
            as unsafe extern "C" fn(
                *mut ::core::ffi::c_void,
                *const XML_Char,
    ...
    parser_set_end_element_handler(Some(
        dummy_end_element as unsafe extern "C" fn(*mut ::core::ffi::c_void, *const XML_Char),
    ));
    parse_single_bytes_c_string_expect_success(text, 3302 as ::core::ffi::c_int);
    char_data_check_xml_chars(&mut storage, expected);
}
extern "C" fn test_ignore_section_utf16() {
    set_test_info(b"test_ignore_section_utf16\0", 3307 as ::core::ffi::c_int);
    let text = *b"<\0!\0D\0O\0C\0T\0Y\0P\0E\0 \0d\0 \0S\0Y\0S\0T\0E\0M\0 \0'\0s\0'\0>\0\n\0<\0d\0>\0<\0e\0>\0&\0e\0n\0;\0<\0/\0e\0>\0<\0/\0d\0>\0\0";
    let expected = bytes_as_xml_char_ptr(b"<![IGNORE[<!ELEMENT e (#PCDATA)*>]]>\n&en;\0");
    let mut storage = CharData {
        count: 0,
        data: [0; 2048],
    ...
    parse_single_bytes_array_expect_success(&text, 3329 as ::core::ffi::c_int);
    char_data_check_xml_chars(&mut storage, expected);
}
extern "C" fn test_ignore_section_utf16_be() {
    set_test_info(
        b"test_ignore_section_utf16_be\0",
        3334 as ::core::ffi::c_int,
    );
    let text = *b"\0<\0!\0D\0O\0C\0T\0Y\0P\0E\0 \0d\0 \0S\0Y\0S\0T\0E\0M\0 \0'\0s\0'\0>\0\n\0<\0d\0>\0<\0e\0>\0&\0e\0n\0;\0<\0/\0e\0>\0<\0/\0d\0>\0";
    let expected = bytes_as_xml_char_ptr(b"<![IGNORE[<!ELEMENT e (#PCDATA)*>]]>\n&en;\0");
    let mut storage = CharData {
        count: 0,
        data: [0; 2048],
    ...
    parse_single_bytes_array_expect_success(&text, 3357 as ::core::ffi::c_int);
    char_data_check_xml_chars(&mut storage, expected);
}
extern "C" fn test_bad_ignore_section() {
    set_test_info(b"test_bad_ignore_section\0", 3363 as ::core::ffi::c_int);
    let text = bytes_as_c_char_ptr(b"<!DOCTYPE doc SYSTEM 'foo'>\n<doc><e>&entity;</e></doc>\0");
    let mut faults = [
        ext_faults {
            parse_text: bytes_as_c_char_ptr(b"<![IGNORE[<!ELEM\0"),
            fail_text: bytes_as_c_char_ptr(b"Broken-off declaration not faulted\0"),
            encoding: ::core::ptr::null::<XML_Char>(),
            error: XML_ERROR_SYNTAX,
        },
        ext_faults {
            parse_text: bytes_as_c_char_ptr(b"<![IGNORE[\x01]]>\0"),
            fail_text: bytes_as_c_char_ptr(b"Invalid XML character not faulted\0"),
            encoding: ::core::ptr::null::<XML_Char>(),
            error: XML_ERROR_INVALID_TOKEN,
        },
        ext_faults {
            parse_text: bytes_as_c_char_ptr(b"<![IGNORE[\xE2\x82\0"),
            fail_text: bytes_as_c_char_ptr(b"Partial XML character not faulted\0"),
            encoding: ::core::ptr::null::<XML_Char>(),
            error: XML_ERROR_PARTIAL_CHAR,
        },
        ext_faults {
    ...
    for fault in faults
        ...
        .take_while(|fault| !fault.parse_text.is_null())
    {
        set_subtest_c_string(fault.parse_text);
        parser_set_param_entity_parsing(XML_PARAM_ENTITY_PARSING_ALWAYS);
        parser_set_external_entity_ref_handler(Some(
            external_entity_faulter
                as unsafe extern "C" fn(
        ...
        parser_set_user_data((fault as *mut ExtFaults).cast());
        expect_failure(
            text,
            XML_ERROR_EXTERNAL_ENTITY_HANDLING,
            b"Incomplete IGNORE section not failed\0",
...
extern "C" fn test_external_bom_consumed() {
    set_test_info(b"test_external_bom_consumed\0", 3437 as ::core::ffi::c_int);
    let text = bytes_as_c_char_ptr(b"<!DOCTYPE doc SYSTEM '004-1.ent'>\n<doc></doc>\n\0");
    let external = bytes_as_c_char_ptr(b"\xEF\xBB\xBF<!ATTLIST doc a1 CDATA 'value'>\0");
    let len = c_string_len(external);

    for split in 0..=len {
        set_split_subtest_message(split);
        let mut testdata = bom_testdata {
            external,
            split,
            nested_callback_happened: XML_FALSE,
        };
        let parser = parser_create();
        if parser.is_null() {
            fail_test(3452 as ::core::ffi::c_int, b"Couldn't create parser\0");
        }

        parser_set_param_entity_parsing_for(parser, XML_PARAM_ENTITY_PARSING_ALWAYS);
        parser_set_external_entity_ref_handler_for(
            parser,
            Some(
        ...
        parser_set_user_data_for(parser, (&raw mut testdata).cast());
        ...
        if bom_testdata(parser)
            ...
            == 0
        {
            fail_test(3461 as ::core::ffi::c_int, b"ref handler not called\0");
        }
        parser_free(parser);
    }
}
extern "C" fn test_external_entity_values() {
    set_test_info(b"test_external_entity_values\0", 3469 as ::core::ffi::c_int);
    let text = bytes_as_c_char_ptr(b"<!DOCTYPE doc SYSTEM '004-1.ent'>\n<doc></doc>\n\0");
    let mut data_004_2 = [
        ext_faults {
            parse_text: bytes_as_c_char_ptr(b"<!ATTLIST doc a1 CDATA 'value'>\0"),
            fail_text: ::core::ptr::null::<::core::ffi::c_char>(),
            encoding: ::core::ptr::null::<XML_Char>(),
            error: XML_ERROR_NONE,
        },
        ext_faults {
            parse_text: bytes_as_c_char_ptr(b"<!ATTLIST $doc a1 CDATA 'value'>\0"),
            fail_text: bytes_as_c_char_ptr(b"Invalid token not faulted\0"),
            encoding: ::core::ptr::null::<XML_Char>(),
            error: XML_ERROR_INVALID_TOKEN,
        },
        ext_faults {
            parse_text: bytes_as_c_char_ptr(b"'wombat\0"),
            fail_text: bytes_as_c_char_ptr(b"Unterminated string not faulted\0"),
            encoding: ::core::ptr::null::<XML_Char>(),
            error: XML_ERROR_UNCLOSED_TOKEN,
        },
        ext_faults {
            parse_text: bytes_as_c_char_ptr(b"\xE2\x82\0"),
            fail_text: bytes_as_c_char_ptr(b"Partial UTF-8 character not faulted\0"),
            encoding: ::core::ptr::null::<XML_Char>(),
            error: XML_ERROR_PARTIAL_CHAR,
        },
        ext_faults {
            parse_text: bytes_as_c_char_ptr(b"<?xml version='1.0' encoding='utf-8'?>\n\0"),
            fail_text: ::core::ptr::null::<::core::ffi::c_char>(),
            encoding: ::core::ptr::null::<XML_Char>(),
            error: XML_ERROR_NONE,
        },
        ext_faults {
            parse_text: bytes_as_c_char_ptr(b"<?xml?>\0"),
            fail_text: bytes_as_c_char_ptr(b"Malformed XML declaration not faulted\0"),
            encoding: ::core::ptr::null::<XML_Char>(),
            error: XML_ERROR_XML_DECL,
        },
        ext_faults {
            parse_text: bytes_as_c_char_ptr(b"\xEF\xBB\xBF<!ATTLIST doc a1 CDATA 'value'>\0"),
            fail_text: ::core::ptr::null::<::core::ffi::c_char>(),
            encoding: ::core::ptr::null::<XML_Char>(),
            error: XML_ERROR_NONE,
        },
        ext_faults {
            parse_text: bytes_as_c_char_ptr(b"<?xml version='1.0' encoding='utf-8'?>\n$\0"),
            fail_text: bytes_as_c_char_ptr(b"Invalid token after text declaration not faulted\0"),
            encoding: ::core::ptr::null::<XML_Char>(),
            error: XML_ERROR_INVALID_TOKEN,
        },
        ext_faults {
            parse_text: bytes_as_c_char_ptr(b"<?xml version='1.0' encoding='utf-8'?>\n'wombat\0"),
            fail_text: bytes_as_c_char_ptr(b"Unterminated string after text decl not faulted\0"),
            encoding: ::core::ptr::null::<XML_Char>(),
            error: XML_ERROR_UNCLOSED_TOKEN,
        },
        ext_faults {
            parse_text: bytes_as_c_char_ptr(b"<?xml version='1.0' encoding='utf-8'?>\n\xE2\x82\0"),
            fail_text: bytes_as_c_char_ptr(
                b"Partial UTF-8 character after text decl not faulted\0",
            ),
            encoding: ::core::ptr::null::<XML_Char>(),
            error: XML_ERROR_PARTIAL_CHAR,
        },
        ext_faults {
            parse_text: bytes_as_c_char_ptr(b"%e1;\0"),
            fail_text: bytes_as_c_char_ptr(b"Recursive parameter entity not faulted\0"),
            encoding: ::core::ptr::null::<XML_Char>(),
        ...
        .take_while(|fault| !fault.parse_text.is_null())
    {
        ...
        parser_set_user_data((fault as *mut ExtFaults).cast());
        parse_single_bytes_c_string_expect_success(text, 3507 as ::core::ffi::c_int);
        parser_reset();
    }
}
extern "C" fn test_ext_entity_not_standalone() {
    set_test_info(
        b"test_ext_entity_not_standalone\0",
        3514 as ::core::ffi::c_int,
    );
    let text = bytes_as_c_char_ptr(b"<!DOCTYPE doc SYSTEM 'foo'>\n<doc></doc>\0");

    parser_set_param_entity_parsing(XML_PARAM_ENTITY_PARSING_ALWAYS);
    parser_set_external_entity_ref_handler(external_entity_not_standalone_handler_for_tests());
    expect_failure(
        text,
        XML_ERROR_EXTERNAL_ENTITY_HANDLING,
        b"Standalone rejection not caught\0",
        3521 as ::core::ffi::c_int,
    );
}
extern "C" fn test_ext_entity_value_abort() {
    set_test_info(b"test_ext_entity_value_abort\0", 3525 as ::core::ffi::c_int);
    let text = bytes_as_c_char_ptr(b"<!DOCTYPE doc SYSTEM '004-1.ent'>\n<doc></doc>\n\0");

    parser_set_param_entity_parsing(XML_PARAM_ENTITY_PARSING_ALWAYS);
    parser_set_external_entity_ref_handler(external_entity_value_aborter_handler_for_tests());
    set_parser_stop_state(XML_FALSE, None);
    parse_single_bytes_c_string_expect_success(text, 3534 as ::core::ffi::c_int);
}
...
extern "C" fn test_attribute_enum_value() {
    set_test_info(b"test_attribute_enum_value\0", 3551 as ::core::ffi::c_int);
    let text = bytes_as_c_char_ptr(b"<?xml version='1.0' standalone='no'?>\n<!DOCTYPE animal SYSTEM 'test.dtd'>\n<animal>This is a \n    <a/>  \n\nyellow tiger</animal>\0");
    ...
    let expected = bytes_as_xml_char_ptr(b"This is a \n      \n\nyellow tiger\0");
    ...
    parser_set_user_data((&raw mut dtd_data).cast());
    parser_set_param_entity_parsing(XML_PARAM_ENTITY_PARSING_ALWAYS);
    parser_set_attlist_decl_handler(Some(
        dummy_attlist_decl_handler
            as unsafe extern "C" fn(
    ...
    run_ext_character_check(text, &mut dtd_data, expected, 3567 as ::core::ffi::c_int);
}
extern "C" fn test_predefined_entity_redefinition() {
    set_test_info(
        b"test_predefined_entity_redefinition\0",
        3576 as ::core::ffi::c_int,
    );
    run_character_check(
        bytes_as_c_char_ptr(b"<!DOCTYPE doc [\n<!ENTITY apos 'foo'>\n]>\n<doc>&apos;</doc>\0"),
        bytes_as_xml_char_ptr(b"'\0"),
        3581 as ::core::ffi::c_int,
    );
}
extern "C" fn test_dtd_stop_processing() {
    set_test_info(b"test_dtd_stop_processing\0", 3588 as ::core::ffi::c_int);
    let text = bytes_as_c_char_ptr(b"<!DOCTYPE doc [\n%foo;\n<!ENTITY bar 'bas'>\n]><doc/>\0");
    ...
    initialize_dummy_handlers();
    parse_single_bytes_c_string_expect_success(text, 3598 as ::core::ffi::c_int);
    if dummy_handler_flags() != 0 as ::core::ffi::c_ulong {
        fail_test(
            3600 as ::core::ffi::c_int,
            b"DTD processing still going after undefined PE\0",
        );
    }
}
extern "C" fn test_public_notation_no_sysid() {
    set_test_info(
        b"test_public_notation_no_sysid\0",
    ...
    initialize_dummy_handlers();
    parser_set_notation_decl_handler(dummy_notation_decl_handler_for_tests());
    parse_single_bytes_c_string_expect_success(text, 3615 as ::core::ffi::c_int);
    assert_dummy_handler_flags(
        DUMMY_NOTATION_DECL_HANDLER_FLAG,
        3617 as ::core::ffi::c_int,
        b"Notation declaration handler not called\0",
    );
}
extern "C" fn test_nested_groups() {
    set_test_info(b"test_nested_groups\0", 3621 as ::core::ffi::c_int);
    let text = bytes_as_c_char_ptr(
    ...
    char_data_init(&mut storage);
    parser_set_element_decl_handler(dummy_element_decl_handler_for_tests());
    parser_set_start_element_handler(record_element_start_handler_for_tests());
    parser_set_user_data((&raw mut storage).cast());
    initialize_dummy_handlers();
    parse_single_bytes_c_string_expect_success(text, 3641 as ::core::ffi::c_int);
    char_data_check_xml_chars(&mut storage, bytes_as_xml_char_ptr(b"doce\0"));
    assert_dummy_handler_flags(
        DUMMY_ELEMENT_DECL_HANDLER_FLAG,
        3644 as ::core::ffi::c_int,
        b"Element handler not fired\0",
    );
}
extern "C" fn test_group_choice() {
    set_test_info(b"test_group_choice\0", 3648 as ::core::ffi::c_int);
    let text = bytes_as_c_char_ptr(
    ...
    parser_set_element_decl_handler(dummy_element_decl_handler_for_tests());
    initialize_dummy_handlers();
    parse_single_bytes_c_string_expect_success(text, 3665 as ::core::ffi::c_int);
    assert_dummy_handler_flags(
        DUMMY_ELEMENT_DECL_HANDLER_FLAG,
        3667 as ::core::ffi::c_int,
        b"Element handler flag not raised\0",
    );
}
...
extern "C" fn test_skipped_parameter_entity() {
    set_test_info(
        b"test_skipped_parameter_entity\0",
    ...
    let mut dtd_data: ExtTest = ExtTest {
        parse_text: bytes_as_c_char_ptr(b"%pe2;\0"),
        encoding: ::core::ptr::null::<XML_Char>(),
        storage: ::core::ptr::null_mut::<CharData>(),
    };
    ...
    parser_set_user_data((&raw mut dtd_data).cast());
    parser_set_param_entity_parsing(XML_PARAM_ENTITY_PARSING_ALWAYS);
    parser_set_skipped_entity_handler(dummy_skip_handler_for_tests());
    initialize_dummy_handlers();
    parse_single_bytes_c_string_expect_success(text, 3706 as ::core::ffi::c_int);
    assert_dummy_handler_flags(
        DUMMY_SKIP_HANDLER_FLAG,
        3708 as ::core::ffi::c_int,
        b"Skip handler not executed\0",
    );
}
extern "C" fn test_recursive_external_parameter_entity() {
    set_test_info(
        b"test_recursive_external_parameter_entity\0",
    ...
    let mut dtd_data: ExtFaults = ext_faults {
        parse_text: bytes_as_c_char_ptr(b"<!ENTITY % pe2 '&#37;pe2;'>\n%pe2;\0"),
        fail_text: bytes_as_c_char_ptr(b"Recursive external parameter entity not faulted\0"),
        encoding: ::core::ptr::null::<XML_Char>(),
        error: XML_ERROR_RECURSIVE_ENTITY_REF,
    };
    ...
    expect_failure(
        text,
        XML_ERROR_EXTERNAL_ENTITY_HANDLING,
        b"Recursive external parameter not spotted\0",
        3727 as ::core::ffi::c_int,
    );
}
extern "C" fn test_undefined_ext_entity_in_external_dtd() {
    set_test_info(
        b"test_undefined_ext_entity_in_external_dtd\0",
    ...
    parser_set_param_entity_parsing(XML_PARAM_ENTITY_PARSING_ALWAYS);
    parser_set_external_entity_ref_handler(handler);
    parser_set_user_data(::core::ptr::null_mut());
    parse_single_bytes_c_string_expect_success(text, 3741 as ::core::ffi::c_int);

    parser_reset();
    parser_set_param_entity_parsing(XML_PARAM_ENTITY_PARSING_ALWAYS);
    parser_set_external_entity_ref_handler(handler);
    parser_set_user_data(current_parser().cast());
    parse_single_bytes_c_string_expect_success(text, 3752 as ::core::ffi::c_int);
}
extern "C" fn test_suspend_xdecl() {
    set_test_info(b"test_suspend_xdecl\0", 3757 as ::core::ffi::c_int);
    let text = shared_test_text(SharedTestText::CharacterData);

    parser_set_xml_decl_handler(suspending_xml_decl_handler_for_tests());
    parser_set_user_data(current_parser().cast());
    set_parser_stop_state(XML_TRUE, None);
    ...
    if parser_parse_c_string(text) as ::core::ffi::c_uint
        != XML_STATUS_SUSPENDED as ::core::ffi::c_int as ::core::ffi::c_uint
    {
        xml_failure(3767 as ::core::ffi::c_int);
    }
    if parser_error_code() as ::core::ffi::c_uint
        != XML_ERROR_NONE as ::core::ffi::c_int as ::core::ffi::c_uint
    {
        xml_failure(3769 as ::core::ffi::c_int);
    }
    if parse_single_bytes_c_string(text) as ::core::ffi::c_uint
        != XML_STATUS_ERROR as ::core::ffi::c_int as ::core::ffi::c_uint
    {
        fail_test(
            ...
            b"Attempt to parse while suspended not faulted\0",
        );
    }
    if parser_error_code() as ::core::ffi::c_uint
        != XML_ERROR_SUSPENDED as ::core::ffi::c_int as ::core::ffi::c_uint
...
extern "C" fn test_abort_epilog() {
    set_test_info(b"test_abort_epilog\0", 3780 as ::core::ffi::c_int);
    let text = bytes_as_c_char_ptr(b"<doc></doc>\n\r\n\0");
    let mut trigger_char = '\r' as i32 as XML_Char;

    parser_set_default_handler(aborting_default_handler_for_tests());
    parser_set_user_data((&mut trigger_char as *mut XML_Char).cast());
    set_parser_stop_state(XML_FALSE, None);

    if parse_single_bytes_c_string(text) as ::core::ffi::c_uint
        != XML_STATUS_ERROR as ::core::ffi::c_int as ::core::ffi::c_uint
    {
        fail_test(3789 as ::core::ffi::c_int, b"Abort not triggered\0");
    }
    if parser_error_code() as ::core::ffi::c_uint
        != XML_ERROR_ABORTED as ::core::ffi::c_int as ::core::ffi::c_uint
    {
        xml_failure(3791 as ::core::ffi::c_int);
    }
}
extern "C" fn test_abort_epilog_2() {
    set_test_info(b"test_abort_epilog_2\0", 3796 as ::core::ffi::c_int);
    let text = bytes_as_c_char_ptr(b"<doc></doc>\n\0");
    ...
    expect_failure(
        text,
        XML_ERROR_ABORTED,
        b"Abort not triggered\0",
        3803 as ::core::ffi::c_int,
    );
}
extern "C" fn test_suspend_epilog() {
    set_test_info(b"test_suspend_epilog\0", 3808 as ::core::ffi::c_int);
    let text = bytes_as_c_char_ptr(b"<doc></doc>\n\0");
    let mut trigger_char = '\n' as i32 as XML_Char;
    ...
    if parse_single_bytes_c_string(text) as ::core::ffi::c_uint
        != XML_STATUS_SUSPENDED as ::core::ffi::c_int as ::core::ffi::c_uint
    {
        xml_failure(3817 as ::core::ffi::c_int);
    }
}
extern "C" fn test_suspend_in_sole_empty_tag() {
    set_test_info(
        b"test_suspend_in_sole_empty_tag\0",
        3821 as ::core::ffi::c_int,
    );
    let text = bytes_as_c_char_ptr(b"<doc/>\0");

    parser_set_end_element_handler(suspending_end_handler_for_tests());
    parser_set_user_data(current_parser().cast());

    let mut rc = parse_single_bytes_c_string(text);
    if rc as ::core::ffi::c_uint == XML_STATUS_ERROR as ::core::ffi::c_int as ::core::ffi::c_uint {
        xml_failure(3829 as ::core::ffi::c_int);
    } else if rc as ::core::ffi::c_uint
        != XML_STATUS_SUSPENDED as ::core::ffi::c_int as ::core::ffi::c_uint
    {
        fail_test(3831 as ::core::ffi::c_int, b"Suspend not triggered\0");
    }
    rc = parser_resume();
    if rc as ::core::ffi::c_uint == XML_STATUS_ERROR as ::core::ffi::c_int as ::core::ffi::c_uint {
        xml_failure(3834 as ::core::ffi::c_int);
    } else if rc as ::core::ffi::c_uint
        != XML_STATUS_OK as ::core::ffi::c_int as ::core::ffi::c_uint
    {
        fail_test(3836 as ::core::ffi::c_int, b"Resume failed\0");
    }
}
...
extern "C" fn test_partial_char_in_epilog() {
    set_test_info(b"test_partial_char_in_epilog\0", 3848 as ::core::ffi::c_int);
    let text = bytes_as_c_char_ptr(b"<doc></doc>\xE2\x82\0");

    ensure_parser_success(
        parse_single_bytes_with_final(text, c_string_len(text), XML_FALSE as ::core::ffi::c_int),
        3854 as ::core::ffi::c_int,
    );
    if !parser_status_is_error(parser_parse_buffer(0 as ::core::ffi::c_int, XML_TRUE as _)) {
        fail_test(
            3857 as ::core::ffi::c_int,
            b"Partial character in epilog not faulted\0",
        );
    }
    if parser_error_code() as ::core::ffi::c_uint
        != XML_ERROR_PARTIAL_CHAR as ::core::ffi::c_int as ::core::ffi::c_uint
    {
        xml_failure(3859 as ::core::ffi::c_int);
    }
}
extern "C" fn test_suspend_resume_internal_entity() {
    set_test_info(
        b"test_suspend_resume_internal_entity\0",
        3864 as ::core::ffi::c_int,
    );
    let text = bytes_as_c_char_ptr(
        b"<!DOCTYPE doc [\n<!ENTITY foo '<suspend>Hi<suspend>Ho</suspend></suspend>'>\n]>\n<doc>&foo;</doc>\n\0",
    );
    let expected1 = bytes_as_xml_char_ptr(b"Hi\0");
    let expected2 = bytes_as_xml_char_ptr(b"HiHo\0");
    let mut storage = CharData {
        count: 0,
        data: [0; 2048],
    };
    ...
    char_data_init(&mut storage);
    parser_set_start_element_handler(suspending_start_element_handler_for_tests());
    parser_set_character_data_handler(accumulating_character_handler());
    parser_set_user_data((&mut storage as *mut CharData).cast());
...
extern "C" fn test_suspend_resume_internal_entity_issue_629() {
    ...
    if parser_parse_for(
        ...
        != XML_STATUS_SUSPENDED as ::core::ffi::c_int as ::core::ffi::c_uint
    {
        xml_failure_for(parser, 3947 as ::core::ffi::c_int);
    }
    if parser_resume_for(parser) as ::core::ffi::c_uint
        != XML_STATUS_OK as ::core::ffi::c_int as ::core::ffi::c_uint
    {
        xml_failure_for(parser, 3949 as ::core::ffi::c_int);
    }
    if parse_single_bytes_with_final_for(
        parser,
        c_string_offset(text, first_chunk_size_bytes),
        c_string_len(text) - first_chunk_size_bytes,
        XML_TRUE as ::core::ffi::c_int,
    ) as ::core::ffi::c_uint
        != XML_STATUS_OK as ::core::ffi::c_int as ::core::ffi::c_uint
    {
        xml_failure_for(parser, 3954 as ::core::ffi::c_int);
    }
    parser_free(parser);
}
extern "C" fn test_resume_entity_with_syntax_error() {
    set_test_info(
        b"test_resume_entity_with_syntax_error\0",
    ...
    if parser_parse_c_string(text) as ::core::ffi::c_uint
        != XML_STATUS_SUSPENDED as ::core::ffi::c_int as ::core::ffi::c_uint
    {
        xml_failure(3976 as ::core::ffi::c_int);
    }
    if parser_resume() as ::core::ffi::c_uint
        != XML_STATUS_ERROR as ::core::ffi::c_int as ::core::ffi::c_uint
    {
...
extern "C" fn test_suspend_resume_parameter_entity() {
    ...
    char_data_init(&mut storage);
    parser_set_param_entity_parsing(XML_PARAM_ENTITY_PARSING_ALWAYS);
    parser_set_element_decl_handler(suspending_element_decl_handler_for_tests());
    parser_set_character_data_handler(accumulating_character_handler());
    parser_set_user_data((&mut storage as *mut CharData).cast());
    ...
    if parser_parse_c_string(text) as ::core::ffi::c_uint
        != XML_STATUS_SUSPENDED as ::core::ffi::c_int as ::core::ffi::c_uint
    {
        xml_failure(4001 as ::core::ffi::c_int);
    }
    char_data_check_xml_chars(&mut storage, bytes_as_xml_char_ptr(b"\0"));
    if parser_resume() as ::core::ffi::c_uint
        != XML_STATUS_OK as ::core::ffi::c_int as ::core::ffi::c_uint
    {
        xml_failure(4004 as ::core::ffi::c_int);
    }
    char_data_check_xml_chars(&mut storage, expected);
}
extern "C" fn test_restart_on_error() {
    set_test_info(b"test_restart_on_error\0", 4010 as ::core::ffi::c_int);
    let text = bytes_as_c_char_ptr(b"<$doc><doc></doc>\0");

    if !parser_status_is_error(parse_single_bytes_c_string(text)) {
        fail_test(
            4015 as ::core::ffi::c_int,
            b"Invalid tag name not faulted\0",
        );
    }
    if parser_error_code() as ::core::ffi::c_uint
        != XML_ERROR_INVALID_TOKEN as ::core::ffi::c_int as ::core::ffi::c_uint
    {
        xml_failure(4017 as ::core::ffi::c_int);
    }
    if !parser_status_is_error(parser_parse(
        ::core::ptr::null::<::core::ffi::c_char>(),
        0 as ::core::ffi::c_int,
        ...
        fail_test(
            ...
            b"Restarting invalid parse not faulted\0",
        );
    }
    if parser_error_code() as ::core::ffi::c_uint
        != XML_ERROR_INVALID_TOKEN as ::core::ffi::c_int as ::core::ffi::c_uint
    {
        xml_failure(4021 as ::core::ffi::c_int);
    }
}
...
extern "C" fn test_trailing_cr_in_att_value() {
    set_test_info(
        b"test_trailing_cr_in_att_value\0",
        4044 as ::core::ffi::c_int,
    );
    let text = bytes_as_c_char_ptr(b"<doc a='value\r'/>\0");
    ensure_parser_success(
        parse_single_bytes_c_string(text),
...
extern "C" fn test_pi_handled_in_default() {
    set_test_info(b"test_pi_handled_in_default\0", 4215 as ::core::ffi::c_int);
    let text = bytes_as_c_char_ptr(b"<?test processing instruction?>\n<doc/>\0");
    let expected = bytes_as_xml_char_ptr(b"<?test processing instruction?>\n<doc/>\0");
    let mut storage = CharData {
        count: 0,
        data: [0; 2048],
    };
...
extern "C" fn test_comment_handled_in_default() {
    set_test_info(
        b"test_comment_handled_in_default\0",
        4231 as ::core::ffi::c_int,
    );
    let text = bytes_as_c_char_ptr(b"<!-- This is a comment -->\n<doc/>\0");
    let expected = bytes_as_xml_char_ptr(b"<!-- This is a comment -->\n<doc/>\0");
    let mut storage = CharData {
        count: 0,
        data: [0; 2048],
    };
...
extern "C" fn test_ext_entity_latin1_utf16le_bom() {
    set_test_info(
        b"test_ext_entity_latin1_utf16le_bom\0",
    ...
    let mut test_data = ExtTest2 {
        parse_text: bytes_as_c_char_ptr(b"\xFF\xFEL \0"),
        parse_len: 4 as ::core::ffi::c_int,
        encoding: bytes_as_xml_char_ptr(b"iso-8859-1\0"),
        storage: ::core::ptr::null_mut::<CharData>(),
    };
    let expected = bytes_as_xml_char_ptr(b"\xC3\xBF\xC3\xBEL \0");
    let mut storage = CharData {
        count: 0,
        data: [0; 2048],
    };

    configure_ext2_character_data_test(&mut test_data, &mut storage);
    ensure_parser_success(
        parse_single_bytes_c_string(text),
...
extern "C" fn test_ext_entity_latin1_utf16be_bom() {
    set_test_info(
        b"test_ext_entity_latin1_utf16be_bom\0",
    ...
    let mut test_data = ExtTest2 {
        parse_text: bytes_as_c_char_ptr(b"\xFE\xFF L\0"),
        parse_len: 4 as ::core::ffi::c_int,
        encoding: bytes_as_xml_char_ptr(b"iso-8859-1\0"),
        storage: ::core::ptr::null_mut::<CharData>(),
    };
    let expected = bytes_as_xml_char_ptr(b"\xC3\xBE\xC3\xBF L\0");
    let mut storage = CharData {
        count: 0,
        data: [0; 2048],
    };
...
extern "C" fn test_ext_entity_latin1_utf16le_bom2() {
    set_test_info(
        b"test_ext_entity_latin1_utf16le_bom2\0",
...
extern "C" fn test_ext_entity_latin1_utf16be_bom2() {
    set_test_info(
        b"test_ext_entity_latin1_utf16be_bom2\0",
...
extern "C" fn test_ext_entity_utf16_be() {
    set_test_info(b"test_ext_entity_utf16_be\0", 4746 as ::core::ffi::c_int);
    let text = bytes_as_c_char_ptr(
        b"<!DOCTYPE doc [\n  <!ENTITY en SYSTEM 'http://example.org/dummy.ent'>\n]>\n<doc>&en;</doc>\0",
    );
    let mut test_data = ExtTest2 {
        parse_text: bytes_as_c_char_ptr(b"<\0e\0/\0>\0\0"),
        parse_len: 8 as ::core::ffi::c_int,
        encoding: bytes_as_xml_char_ptr(b"utf-16be\0"),
        storage: ::core::ptr::null_mut::<CharData>(),
    };
    let expected = bytes_as_xml_char_ptr(b"\xE3\xB0\x80\xE6\x94\x80\xE2\xBC\x80\xE3\xB8\x80\0");
    let mut storage = CharData {
        count: 0,
        data: [0; 2048],
    };
...
extern "C" fn test_ext_entity_utf16_le() {
    set_test_info(b"test_ext_entity_utf16_le\0", 4775 as ::core::ffi::c_int);
    let text = bytes_as_c_char_ptr(
        b"<!DOCTYPE doc [\n  <!ENTITY en SYSTEM 'http://example.org/dummy.ent'>\n]>\n<doc>&en;</doc>\0",
    );
    let mut test_data = ExtTest2 {
        parse_text: bytes_as_c_char_ptr(b"\0<\0e\0/\0>\0"),
        parse_len: 8 as ::core::ffi::c_int,
        encoding: bytes_as_xml_char_ptr(b"utf-16le\0"),
        storage: ::core::ptr::null_mut::<CharData>(),
    };
    let expected = bytes_as_xml_char_ptr(b"\xE3\xB0\x80\xE6\x94\x80\xE2\xBC\x80\xE3\xB8\x80\0");
    let mut storage = CharData {
...
extern "C" fn test_ext_entity_utf16_unknown() {
    set_test_info(
        b"test_ext_entity_utf16_unknown\0",
    ...
    let mut test_data = ExtFaults2 {
        parse_text: bytes_as_c_char_ptr(b"a\0b\0c\0\0"),
        parse_len: 6 as ::core::ffi::c_int,
        fail_text: bytes_as_c_char_ptr(b"Invalid character in entity not faulted\0"),
        encoding: ::core::ptr::null::<XML_Char>(),
        error: XML_ERROR_INVALID_TOKEN,
    };

    configure_ext2_fault_test(&mut test_data);
    expect_failure(
        text,
        XML_ERROR_EXTERNAL_ENTITY_HANDLING,
        b"Invalid character should not have been accepted\0",
        4822 as ::core::ffi::c_int,
    );
}
extern "C" fn test_ext_entity_utf8_non_bom() {
    set_test_info(
        b"test_ext_entity_utf8_non_bom\0",
    ...
    let mut test_data = ExtTest2 {
        parse_text: bytes_as_c_char_ptr(b"\xEF\xBB\x80\0"),
        parse_len: 3 as ::core::ffi::c_int,
        encoding: ::core::ptr::null::<XML_Char>(),
        storage: ::core::ptr::null_mut::<CharData>(),
    };
    let expected = bytes_as_xml_char_ptr(b"\xEF\xBB\x80\0");
    let mut storage = CharData {
        count: 0,
        data: [0; 2048],
    };
...
extern "C" fn test_utf8_in_cdata_section() {
    set_test_info(b"test_utf8_in_cdata_section\0", 4855 as ::core::ffi::c_int);
    let text = bytes_as_c_char_ptr(b"<doc><![CDATA[one \xC3\xA9 two]]></doc>\0");
    let expected = bytes_as_xml_char_ptr(b"one \xC3\xA9 two\0");

    run_character_check(text, expected, 4863 as ::core::ffi::c_int);
}
extern "C" fn test_utf8_in_cdata_section_2() {
    set_test_info(
        b"test_utf8_in_cdata_section_2\0",
        4868 as ::core::ffi::c_int,
    );
    let text = bytes_as_c_char_ptr(b"<doc><![CDATA[\xC3\xA9]\xC3\xA9two]]></doc>\0");
    let expected = bytes_as_xml_char_ptr(b"\xC3\xA9]\xC3\xA9two\0");
...
extern "C" fn test_trailing_spaces_in_elements() {
    set_test_info(
        b"test_trailing_spaces_in_elements\0",
    ...
    char_data_init(&mut storage);
    parser_set_start_element_handler(record_element_start_handler_for_tests());
    parser_set_end_element_handler(record_element_end_handler_for_tests());
    parser_set_user_data((&mut storage as *mut CharData).cast());
    ensure_parser_success(
        parse_single_bytes_c_string(text),
...
extern "C" fn test_utf16_attribute() {
    set_test_info(b"test_utf16_attribute\0", 5003 as ::core::ffi::c_int);
    let text = *b"<\0d\0 \0\x04\x0E\x08\x0E=\0'\0a\0'\0/\0>\0\0";
    let expected = bytes_as_xml_char_ptr(b"a\0");
    let mut storage = CharData {
        count: 0,
        data: [0; 2048],
    };
    ...
    parser_set_user_data((&raw mut storage).cast());
    parse_single_bytes_array_expect_success(&text, 5018 as ::core::ffi::c_int);
    char_data_check_xml_chars(&mut storage, expected);
}
extern "C" fn test_utf16_second_attr() {
    set_test_info(b"test_utf16_second_attr\0", 5023 as ::core::ffi::c_int);
    let text = *b"<\0d\0 \0a\0=\0'\x001\0'\0 \0\x04\x0E\x08\x0E=\0'\x002\0'\0/\0>\0\0";
    let expected = bytes_as_xml_char_ptr(b"1\0");
    let mut storage = CharData {
        count: 0,
        data: [0; 2048],
    };
    ...
    parser_set_user_data((&raw mut storage).cast());
    parse_single_bytes_array_expect_success(&text, 5038 as ::core::ffi::c_int);
    char_data_check_xml_chars(&mut storage, expected);
}
...
extern "C" fn test_utf16_pe() {
    set_test_info(b"test_utf16_pe\0", 5050 as ::core::ffi::c_int);
    ...
    let expected = bytes_as_xml_char_ptr(b"\xE0\xB8\x84\xE0\xB8\x88=<!ELEMENT doc (#PCDATA)>\n\0");
    let mut storage = CharData {
        count: 0,
        data: [0; 2048],
    };
    ...
    parse_single_bytes_array_expect_success(&text, 5080 as ::core::ffi::c_int);
    char_data_check_xml_chars(&mut storage, expected);
}
...
extern "C" fn test_bad_attr_desc_keyword_utf16() {
    set_test_info(
        b"test_bad_attr_desc_keyword_utf16\0",
    ...
    if parse_single_bytes_array(&text) as ::core::ffi::c_uint
        != XML_STATUS_ERROR as ::core::ffi::c_int as ::core::ffi::c_uint
    {
        fail_test(
            5117 as ::core::ffi::c_int,
            b"Invalid UTF16 attribute keyword not faulted\0",
        );
    }
    if parser_error_code() as ::core::ffi::c_uint
        != XML_ERROR_SYNTAX as ::core::ffi::c_int as ::core::ffi::c_uint
    {
        xml_failure(5119 as ::core::ffi::c_int);
    }
}
...
extern "C" fn test_entity_in_utf16_be_attr() {
    set_test_info(
        b"test_entity_in_utf16_be_attr\0",
        5207 as ::core::ffi::c_int,
    );
    let text = *b"\0<\0e\0 \0a\0=\0'\0&\0#\x002\x002\08\0;\0 \0&\0#\0x\x000\x000\0E\x004\0;\0'\0>\0<\0/\0e\0>\0";
    let expected = bytes_as_xml_char_ptr(b"\xC3\xA4 \xC3\xA4\0");
    let mut storage = CharData {
        count: 0,
        data: [0; 2048],
    };
    ...
    parse_single_bytes_array_expect_success(&text, 5224 as ::core::ffi::c_int);
    char_data_check_xml_chars(&mut storage, expected);
}
extern "C" fn test_entity_in_utf16_le_attr() {
    set_test_info(
        b"test_entity_in_utf16_le_attr\0",
        5229 as ::core::ffi::c_int,
    );
    let text = *b"<\0e\0 \0a\0=\0'\0&\0#\x002\x002\08\0;\0 \0&\0#\0x\x000\x000\0E\x004\0;\0'\0>\0<\0/\0e\0>\0\0";
    let expected = bytes_as_xml_char_ptr(b"\xC3\xA4 \xC3\xA4\0");
    let mut storage = CharData {
        count: 0,
        data: [0; 2048],
    ...
    parse_single_bytes_array_expect_success(&text, 5246 as ::core::ffi::c_int);
    char_data_check_xml_chars(&mut storage, expected);
}
extern "C" fn test_entity_public_utf16_be() {
    run_public_utf16_entity_test(
        b"test_entity_public_utf16_be\0",
...
extern "C" fn test_empty_element_abort() {
    set_test_info(b"test_empty_element_abort\0", 5419 as ::core::ffi::c_int);
    let text = bytes_as_c_char_ptr(b"<abort/>\0");
    parser_set_start_element_handler(start_element_suspender_for_tests());
    if !parser_status_is_error(parse_single_bytes_c_string(text)) {
        fail_test(5425 as ::core::ffi::c_int, b"Expected to error on abort\0");
    }
}
extern "C" fn test_pool_integrity_with_unfinished_attr() {
    set_test_info(
        b"test_pool_integrity_with_unfinished_attr\0",
        5432 as ::core::ffi::c_int,
    );
    let text = bytes_as_c_char_ptr(
        b"<?xml version='1.0' encoding='UTF-8'?>\n<!DOCTYPE foo [\n<!ELEMENT foo ANY>\n<!ENTITY % entp SYSTEM \"external.dtd\">\n%entp;\n]>\n<a></a>\n\0",
    );
    let expected = bytes_as_xml_char_ptr(b"COMMENT\0");
    let mut storage = CharData {
        count: 0,
        data: [0; 2048],
    };
    ...
    char_data_init(&mut storage);
    parser_set_param_entity_parsing(XML_PARAM_ENTITY_PARSING_ALWAYS);
    parser_set_external_entity_ref_handler(Some(external_entity_unfinished_attlist));
    parser_set_attlist_decl_handler(Some(dummy_attlist_decl_handler));
    parser_set_comment_handler(Some(accumulate_comment));
    parser_set_user_data((&mut storage as *mut CharData).cast());
    ensure_parser_success(
        parse_single_bytes_c_string(text),
...
extern "C" fn test_entity_ref_no_elements() {
    set_test_info(b"test_entity_ref_no_elements\0", 5457 as ::core::ffi::c_int);
    let text = bytes_as_c_char_ptr(b"<!DOCTYPE foo [\n<!ENTITY e1 \"test\">\n]> <foo>&e1;\0");
    let parser = create_parser_or_fail(5464 as ::core::ffi::c_int);
        ...
        != XML_ERROR_NO_ELEMENTS as ::core::ffi::c_int as ::core::ffi::c_uint
    {
        fail_test(
            ...
            b"check failed: XML_GetErrorCode(parser) == XML_ERROR_NO_ELEMENTS\0",
        );
    }
    parser_free(parser);
}
