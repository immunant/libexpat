...
pub struct prolog_state {
    pub handler: Option<unsafe fn(*mut prolog_state, c_int, &[c_char], *const ENCODING) -> c_int>,
...
pub type PROLOG_HANDLER = unsafe fn(*mut PROLOG_STATE, c_int, &[c_char], *const ENCODING) -> c_int;
...
unsafe fn prolog0(
    ...
    match tok {
        XML_TOK_PROLOG_S => {
            (*state).handler = Some(
                prolog1 as unsafe fn(*mut PROLOG_STATE, c_int, &[c_char], *const ENCODING) -> c_int,
            );
        ...
        XML_TOK_DECL_OPEN => {
            ...
            ) == 0)
            {
                (*state).handler = Some(
                    doctype0
                        as unsafe fn(*mut PROLOG_STATE, c_int, &[c_char], *const ENCODING) -> c_int,
                );
        ...
        XML_TOK_INSTANCE_START_1 => {
            (*state).handler = Some(
                error as unsafe fn(*mut PROLOG_STATE, c_int, &[c_char], *const ENCODING) -> c_int,
            );
...
unsafe fn doctype1(
    ...
    match tok {
        ...
        XML_TOK_DECL_CLOSE => {
            (*state).handler = Some(
                prolog2 as unsafe fn(*mut PROLOG_STATE, c_int, &[c_char], *const ENCODING) -> c_int,
            );
...
unsafe fn externalSubset0(
    ...
    let _ = (ptr, end);
    (*state).handler = Some(
        externalSubset1 as unsafe fn(*mut PROLOG_STATE, c_int, &[c_char], *const ENCODING) -> c_int,
    );
...
unsafe fn entity0(
    ...
    match tok {
        ...
        XML_TOK_PERCENT => {
            (*state).handler = Some(
                entity1 as unsafe fn(*mut PROLOG_STATE, c_int, &[c_char], *const ENCODING) -> c_int,
            );
            return XML_ROLE_ENTITY_NONE;
        }
        XML_TOK_NAME => {
            (*state).handler = Some(
                entity2 as unsafe fn(*mut PROLOG_STATE, c_int, &[c_char], *const ENCODING) -> c_int,
            );
...
unsafe fn entity1(
    ...
    match tok {
        ...
        XML_TOK_NAME => {
            (*state).handler = Some(
                entity7 as unsafe fn(*mut PROLOG_STATE, c_int, &[c_char], *const ENCODING) -> c_int,
            );
...
unsafe fn entity3(
    ...
    match tok {
        ...
        XML_TOK_LITERAL => {
            (*state).handler = Some(
                entity4 as unsafe fn(*mut PROLOG_STATE, c_int, &[c_char], *const ENCODING) -> c_int,
            );
...
unsafe fn entity4(
    ...
    match tok {
        ...
        XML_TOK_LITERAL => {
            (*state).handler = Some(
                entity5 as unsafe fn(*mut PROLOG_STATE, c_int, &[c_char], *const ENCODING) -> c_int,
            );
...
unsafe fn entity8(
    ...
    match tok {
        ...
        XML_TOK_LITERAL => {
            (*state).handler = Some(
                entity9 as unsafe fn(*mut PROLOG_STATE, c_int, &[c_char], *const ENCODING) -> c_int,
            );
            return XML_ROLE_ENTITY_PUBLIC_ID;
        }
        _ => {}
...
unsafe fn common(mut state: *mut PROLOG_STATE, mut tok: c_int) -> c_int {
    ...
    (*state).handler =
        Some(error as unsafe fn(*mut PROLOG_STATE, c_int, &[c_char], *const ENCODING) -> c_int);
    return XML_ROLE_ERROR;
}
pub(crate) unsafe fn XmlPrologStateInit(mut state: *mut PROLOG_STATE) {
    (*state).handler =
        Some(prolog0 as unsafe fn(*mut PROLOG_STATE, c_int, &[c_char], *const ENCODING) -> c_int);
...
pub(crate) unsafe fn XmlPrologStateInitExternalEntity(mut state: *mut PROLOG_STATE) {
    (*state).handler = Some(
        externalSubset0 as unsafe fn(*mut PROLOG_STATE, c_int, &[c_char], *const ENCODING) -> c_int,
    );
