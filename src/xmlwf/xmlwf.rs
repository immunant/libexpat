    ...
    unsafe {
        ...
        if nAtts > 1 {
            qsort(
                ...
                Some(attcmp as extern "C" fn(*const c_void, *const c_void) -> c_int),
            );
        }
        while !(*atts).is_null() {
            putc(' ' as i32, fp);
    ...
    unsafe {
        ...
        if nAtts > 1 {
            qsort(
                ...
                nAtts as size_t,
                (size_of::<*mut XML_Char>()).wrapping_mul(2usize),
                Some(nsattcmp as extern "C" fn(*const c_void, *const c_void) -> c_int),
            );
        }
        while !(*atts).is_null() {
            let fresh1 = atts;
        ...
        if notationCount != 0 {
            ...
            if notations.is_null() {
                ...
                qsort(
                    ...
                    Some(notationCmp as extern "C" fn(*const c_void, *const c_void) -> c_int),
                );
...
extern "C" fn unknownEncoding(
    ...
    unsafe {
        let mut cp: c_int = 0;
        static prefixL: [XML_Char; 9] =
            unsafe { transmute::<[u8; 9], [XML_Char; 9]>(*b"windows-\0") };
        static prefixU: [XML_Char; 9] =
            unsafe { transmute::<[u8; 9], [XML_Char; 9]>(*b"WINDOWS-\0") };
        let mut i: c_int = 0;
        i = 0;
